      ...
      <nav className="navbar">
        <div className="logo-container">
          ...
          <div className="logo-text">
            <Link to="/employer" onClick={handleLinkClick}>ITIAN</Link>
          </div>
        </div>

        <div className={`menu ${isMobileMenuOpen ? "open" : ""}`}>
           <Link to="/employer" className="nav-link" onClick={handleLinkClick}>
              <FaHome className="nav-icon" /> {t('navbar.home') || 'Home'}
            </Link>
           <Link to="/employer-profile" className="nav-link" onClick={handleLinkClick}>
            <FaUser className="nav-icon" /> {t('navbar.profile') || 'My Profile'}
          </Link>
          
          <Link to="/employer/mychat" className="nav-link" onClick={handleLinkClick}>
          ...
          <div className={`dropdown ${openDropdown === 'jobs' ? 'open' : ''}`}>
            ...
            {openDropdown === 'jobs' && (
              <div className="dropdown-content">
                <Link to="/payment" onClick={handleLinkClick}><FaFileAlt className="dropdown-icon" /> {t('navbar.postJob') || 'Post New Job'}</Link>
                <Link to="/employer/jobs" onClick={handleLinkClick}><FaFileAlt className="dropdown-icon" /> {t('navbar.myJobs') || 'My Jobs'}</Link>
              </div>
            )}
          </div>
          ...
          <div className={`dropdown ${openDropdown === 'reports' ? 'open' : ''}`}>
            ...
            {openDropdown === 'reports' && (
              <div className="dropdown-content">
                <Link to="/employer/reports/create" onClick={handleLinkClick}><FaFileAlt className="dropdown-icon" /> {t('navbar.createReport') || 'Create Report'}</Link>
                <Link to="/employer/my-reports" onClick={handleLinkClick}><FaFileAlt className="dropdown-icon" /> {t('navbar.myReports') || 'My Reports'}</Link>
              </div>
            )}
          </div>
