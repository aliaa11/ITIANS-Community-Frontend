...
function AdminSetPricePage() {
  ...
  useEffect(() => {
    const fetchPrice = async () => {
      try {
        ...
        const res = await axios.get('https://c6a0-41-33-92-146.ngrok-free.appapi/job-price', {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    const token = localStorage.getItem('access-token');
    try {
      await axios.post('https://itians-community-backend-production.up.railway.app/api/set-job-price', { price }, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
